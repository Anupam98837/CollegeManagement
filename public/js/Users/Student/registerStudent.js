...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const student_Phone = sessionStorage.getItem("student_phone");
    const student_Email = sessionStorage.getItem("student_email");

    if (token) {
        document.getElementById("studentName").value = student_Name;
        document.getElementById("phone").value = student_Phone;
        document.getElementById("email").value = student_Email;
        fetchStudentData(student_Email);
    }
});
      ...
      .then(data => {
          if (data.status === 'success') {
              ...
              Swal.fire({
                  ...
                  window.location.href = '/student/student_ragister';
              });
          }
      })
      .catch(error => {
...
function formatDate(dateString) {
    if (!dateString) return ""; // If date is null or undefined, return an empty string
...
function populateForm(student) {
    // console.log(student);
    if(student){
        document.querySelector(".ragister_complete_message_div").classList.remove("d-none");
        document.querySelector(".admin_ragister_student_form").style.display = "none";
    }
}

// Function to populate the marks table dynamically
function populateMarksTable(tableId, classType, jsonData) {
    ...
    try {
        ...
        data.forEach(entry => {
            addRowWithData(tableId, classType, entry.subject, entry.fullMarks, entry.marksObtained, entry.percentage);
        });
    } catch (error) {
        console.error(`Error parsing ${classType} data:`, error);
    }
}

// Function to add a row with pre-filled data
function addRowWithData(tableId, classType, subject, fullMarks, marksObtained, percentage) {
    const tableBody = document.getElementById(tableId);
    const newRow = document.createElement('tr');

    newRow.innerHTML = `
        <td><input type="text" class="form-control text-13 placeholder-14" value="${subject}" required></td>
        <td><input type="number" class="form-control text-13 placeholder-14" value="${fullMarks}" required></td>
        <td><input type="number" class="form-control text-13 placeholder-14" value="${marksObtained}" required></td>
        <td><input type="text" class="form-control text-13 placeholder-14" value="${percentage}" readonly></td>
...
// NEW: Function to change directly to a specified step.
...
document.querySelector(".admin_ragister_student_form").addEventListener("submit", function (event) {
    ...
    // Implement missing required fields check
    ...
    if (missingInputs.length > 0) {
        // Focus on the first missing input
        missingInputs[0].focus();

        // Determine the step containing the missing field and redirect to that step
        let stepElement = missingInputs[0].closest(".step");
        if (stepElement && stepElement.id) {
            // Assumes step id is in the format "step-1", "step-2", etc.
            const stepNumber = parseInt(stepElement.id.split("-")[1]);
            changeStepTo(stepNumber);
        }

        // Build a list of missing field names (using label text if available)
        const missingFieldNames = Array.from(missingInputs).map(input => {
            const label = form.querySelector(`label[for="${input.id}"]`);
            return label ? label.textContent : (input.name || "Field");
        });

        // Append error message below each missing input
        ...
        Swal.fire("Missing Required Fields", "Please fill out: " + missingFieldNames.join(", "), "warning");
        return; // Do not proceed with submission
    }

    const formData = new FormData(this);
    const action = this.dataset.action || "register";
    const apiUrl = action === "update" ? "/api/update-student" : "/api/register-student";

    const submitBtn = document.getElementById("submitBtn");
    const originalBtnText = action === "update" ? "Update Details" : "Submit";
    ...
    .then((data) => {
        if (data.status === "success") {
            Swal.fire("Success", data.message, "success").then(() => {
                if (apiUrl === "/api/register-student") {
                    window.location.href = "/student/upload-Document";
                } else {
